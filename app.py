...
# Streamlit app
def main():
    st.title("Word Embedding and Similarity")

    # Input for QA
    f = st.text_area("Enter First Sentence:")
    s = st.text_area("Enter Second Sentence:")
    ...
    if st.button("Visualize Embeddings"):
        embeddings = encode_sentence(s)  # Use the entered question for embeddings
        tokens = tokenizer_emb.tokenize(tokenizer_emb.decode(tokenizer_emb.encode(s)))
        st.header("Visualization of Word Embeddings")
        visualize_word_embeddings(embeddings[0].detach().numpy(), tokens)

    # Calculate cosine similarity
    st.header("Similarity of Sentences")
    context_embeddings = encode_sentence_sim(f)
    question_embeddings = encode_sentence_sim(s)
